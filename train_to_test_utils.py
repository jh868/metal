...
def train(train_loader, epoch_num, model, device, criterion, optimizer, scheduler):
...
def test(test_loader, device, label_quantity):
    ...
    model.classifier[1] = nn.Linear(in_features=2048, out_features=10)
    model.load_state_dict(torch.load('./{}_20_epoch.pt'.format(model._get_name()), map_location=device))
    model.to(device)
